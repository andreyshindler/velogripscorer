...
const I18N = {
  en: {
    ...
    import_csv: 'Import CSV', import_done: 'Imported {n} racers ({s} skipped)',
    csv_help: 'CSV columns: bib, name, category, wave, epc — header row optional, EPC optional (bib-based chip is generated).',
    tab_manage: 'Manage', app_pairing: 'Timing app connection',
    app_pairing_help: 'Paste this code and this website\'s address into the VeloGrip RFID Android app (Settings → Server URL + Reader device token). The app then downloads the start list and uploads race results.',
  ...
  he: {
    ...
    import_csv: 'ייבוא CSV', import_done: 'יובאו {n} רוכבים ({s} דולגו)',
    csv_help: 'עמודות ה-CSV: מספר חזה, שם, קטגוריה, מקצה, EPC — שורת כותרת רשות, EPC רשות (ייווצר שבב לפי מספר החזה).',
    tab_manage: 'ניהול', app_pairing: 'חיבור אפליקציית המדידה',
    app_pairing_help: 'הדבק קוד זה ואת כתובת האתר באפליקציית VeloGrip RFID (הגדרות ← כתובת השרת + אסימון מכשיר). האפליקציה תוריד את רשימת הזינוק ותעלה את תוצאות המרוץ.',
